...
- "spotkanie o 15:00" → {"activity": "spotkanie", "datetime": "${today}T15:00:00.000"} (jeśli teraz jest przed 15:00)
- "spotkanie o 15:00" → {"activity": "spotkanie", "datetime": "${tomorrow}T15:00:00.000"} (jeśli teraz jest po 15:00)
...
- "za tydzień o tej porze" → {"activity": "[aktywność]", "datetime": "[data za 7 dni o tej samej godzinie]"}
- "za 24 godziny" → {"activity": "[aktywność]", "datetime": "[data dokładnie za 24 godziny]"}
- "za minutę" → {"activity": "[aktywność]", "datetime": "${new Date(now.getTime() + 1 * 60 * 1000).toISOString().slice(0,19)}.000"}
- "za jedną godzinę" → {"activity": "[aktywność]", "datetime": "${plus1hStr}"}
- "za dwie godziny" → {"activity": "[aktywność]", "datetime": "${plus2hStr}"}
- "za 2 godziny i 30 minut" → {"activity": "[aktywność]", "datetime": "${new Date(now.getTime() + (2 * 60 + 30) * 60 * 1000).toISOString().slice(0,19)}.000"}
- "o północy" → {"activity": "[aktywność]", "datetime": "${today}T00:00:00.000"} (jeśli północ jeszcze nie minęła, w przeciwnym razie jutro)
- "o szóstej rano" → {"activity": "[aktywność]", "datetime": "${today}T06:00:00.000"} (jeśli szósta rano jeszcze nie minęła, w przeciwnym razie jutro)
- "o 23:59" → {"activity": "[aktywność]", "datetime": "${today}T23:59:00.000"} (jeśli jeszcze nie minęła, w przeciwnym razie jutro)
- "za 48 godzin" → {"activity": "[aktywność]", "datetime": "[data za 2 dni o tej samej godzinie]"}
