...
import { LLMParserService, LLMAbuseResult } from '../services/llm_parser_service';
...
export class CreateReminderHandler {
  ...
  async execute(
  ...
  ): Promise<CreateReminderResult | CreateReminderAbuseResult> {
    ...
    if (llmResult.activity && llmResult.activity.length > MAX_ACTIVITY_LENGTH) {
      throw new ValidationError(
        `Aktywność przekracza maksymalną długość ${MAX_ACTIVITY_LENGTH} znaków`
      );
    }
